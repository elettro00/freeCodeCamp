...
for instance mergeSort, which is responsible for the recursion that produces single-item arrays to feed into merge.
Good luck!
...
function main(lxArr, rxArr) {
    let res = []
    while (lxArr.length > 0 && rxArr.length > 0) {
        if (lxArr[0] < rxArr[0]) {
            res.push(lxArr.shift())  
        } else {
            res.push(rxArr.shift()) 
        }
    }
    return [ ...res, ...lxArr, ...rxArr ]
}
...
function mergeSort(array) {
    ...
    let lxArr = array.splice(0, middle)
    let rxArr = array
    return main(mergeSort(lxArr),mergeSort(rxArr))
  }
